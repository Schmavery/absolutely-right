...
/** `RUN_PHASE2_BENCH=1 vitest run tests/launchBench.test.ts -t phase` */
describe.skipIf(!process.env.RUN_PHASE2_BENCH)('launch time (phase2 smoke)', () => {
  it(`progress reaches flavor phase ${BENCH_PHASE_2} on seed 42 within 10h`, () => {
...
describe.skipIf(!process.env.RUN_LAUNCH_BENCH)('launch time (bench)', () => {
  ...
  it('median launch adaptive × patience + rank bots', () => {
...
describe.skipIf(!process.env.RUN_PHASE2_BENCH)('phase 2 time (bench)', () => {
  it('median phase 2 adaptive × patience + rank bots', () => {
